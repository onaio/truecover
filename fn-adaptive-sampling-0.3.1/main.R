...
handle_error = function(error) {
  message(error)
  type = 'error'
  function_response = toJSON(list(function_status = unbox(type), result = unbox(unbox(error$message))))
  return(write(function_response, stdout()))
}

handle_success = function(content) {
  type = 'success'
  function_response = toJSON(list(function_status = unbox(type), result = content))
  return(write(function_response, stdout()))
}
